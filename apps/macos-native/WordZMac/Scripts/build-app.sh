...
if [[ "${WORDZ_MAC_DISABLE_SWIFTPM_SANDBOX:-0}" == "1" ]]; then
  SWIFTPM_SANDBOX_ARGS+=(--disable-sandbox)
fi
...
swift build "${SWIFTPM_SANDBOX_ARGS[@]}" --package-path "$APP_ROOT" -c release --product "$SWIFT_PRODUCT"

BIN_PATH=$(
  HOME="$LOCAL_HOME" CLANG_MODULE_CACHE_PATH="$LOCAL_CLANG_CACHE" SWIFTPM_MODULECACHE_OVERRIDE="$LOCAL_SWIFTPM_CACHE" \
  swift build "${SWIFTPM_SANDBOX_ARGS[@]}" --package-path "$APP_ROOT" -c release --product "$SWIFT_PRODUCT" --show-bin-path
)
