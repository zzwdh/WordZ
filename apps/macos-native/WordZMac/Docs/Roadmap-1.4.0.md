...
| API 调用底座 | 78% | 已有 `NativeAPIClient` 第一版，更新检查和 API 连接测试已迁移，API 关闭时会阻止联网更新动作，queued / in-flight 取消测试已覆盖，更新检查 service factory 已接入当前 API timeout/concurrency 设置，27 个 API/update/settings focused tests 通过 | 继续补一个窄 API 试点 |
