...
package enum SQLiteJournalMode: String {
    case delete = "DELETE"
    case wal = "WAL"
}

package enum SQLiteTransactionMode {
    case deferred
    case immediate
    case exclusive
...
package struct SQLiteDatabaseConfiguration {
    package let journalMode: SQLiteJournalMode
    package let synchronousMode: String
    package let foreignKeysEnabled: Bool
    package let busyTimeoutMS: Int32

    package static let libraryCatalog = SQLiteDatabaseConfiguration(
        journalMode: .wal,
    ...
    package static let workspaceState = SQLiteDatabaseConfiguration(
        journalMode: .wal,
        synchronousMode: "NORMAL",
        foreignKeysEnabled: true,
        busyTimeoutMS: 5_000
    ...
    package static let corpusShard = SQLiteDatabaseConfiguration(
        journalMode: .delete,
        synchronousMode: "NORMAL",
...
package final class SQLiteDatabase {
    ...
    package func tableExists(_ tableName: String) throws -> Bool {
    ...
    package func columnExists(_ columnName: String, onTable tableName: String) throws -> Bool {
    ...
    package func scalarText(_ sql: String) throws -> String? {
        let statement = try prepare(sql)
        guard statement.step() == SQLITE_ROW else {
            return nil
        }
    ...
    package func checkpointIfNeeded() {
    ...
    package func error(message: String) -> NSError {
        NSError(
            domain: "WordZMac.SQLiteDatabase",
    ...
    package static func atomicReplaceItem(
    ...
    package static func databaseSidecarURLs(for databaseURL: URL) -> [URL] {
        [
    ...
    package static func removeDatabaseSidecars(
        for databaseURL: URL,
        fileManager: FileManager = .default
    ) {
    ...
    package static func backupDatabase(
        from sourceURL: URL,
...
package final class SQLiteStatement {
    ...
    package init(database: SQLiteDatabase, sql: String) throws {
    ...
    @discardableResult
    package func step() -> Int32 {
        sqlite3_step(statement)
    }

    package func bind(text value: String, at index: Int32) {
    ...
    package func bind(optionalText value: String?, at index: Int32) {
        if let value {
    ...
    package func bind(int value: Int, at index: Int32) {
        sqlite3_bind_int64(statement, index, sqlite3_int64(value))
    }

    package func bind(double value: Double, at index: Int32) {
        sqlite3_bind_double(statement, index, value)
    }

    package func bind(bool value: Bool, at index: Int32) {
        bind(int: value ? 1 : 0, at: index)
    }

    package func bindNull(at index: Int32) {
    ...
    package func double(at index: Int32) -> Double {
        sqlite3_column_double(statement, index)
    }

    package func bool(at index: Int32) -> Bool {
        int(at: index) != 0
    }

    package func text(at index: Int32) -> String {
