...
extension NativeTopicEngine {
    ...
    func buildKeywordBigrams(from terms: [String]) -> [String] {
        ...
        var bigrams: [String] = []
        var seen = Set<String>()
        for distance in 1...min(2, terms.count - 1) {
            for index in 0..<(terms.count - distance) {
                let bigram = "\(terms[index]) \(terms[index + distance])"
                guard seen.insert(bigram).inserted else { continue }
                bigrams.append(bigram)
            }
        }
        return Array(bigrams.prefix(8))
    }
