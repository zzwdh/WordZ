...
enum NativeWindowRouting {
    ...
    static func identifier(for route: NativeWindowRoute) -> NSUserInterfaceItemIdentifier {
        NSUserInterfaceItemIdentifier(route.id)
    }

    static func register(_ window: NSWindow?, for route: NativeWindowRoute) {
    ...
    static func window(for route: NativeWindowRoute) -> NSWindow? {
    ...
    static func isActive(_ route: NativeWindowRoute) -> Bool {
        let application = NSApplication.shared
        let identifier = identifier(for: route)
        return application.keyWindow?.identifier == identifier || application.mainWindow?.identifier == identifier
    }

    static func presentationWindow(preferredRoute: NativeWindowRoute?) -> NSWindow? {
