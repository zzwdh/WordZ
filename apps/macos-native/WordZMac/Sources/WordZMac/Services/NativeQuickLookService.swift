...
@MainActor
protocol NativeQuickLookServicing: AnyObject {
    func preview(path: String) throws
}

@MainActor
final class NativeQuickLookService: NSObject, NativeQuickLookServicing {
    nonisolated(unsafe) private var previewItems: [NSURL] = []

    func preview(path: String) throws {
        let trimmedPath = path.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmedPath.isEmpty else {
            throw NSError(
                domain: "WordZMac.NativeQuickLookService",
                code: 1,
