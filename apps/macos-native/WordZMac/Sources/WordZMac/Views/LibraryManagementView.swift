    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            LazyVGrid(columns: [GridItem(.adaptive(minimum: 150), spacing: 12)], spacing: 12) {
                ...
                WorkbenchMetricCard(title: "Sentences", value: scene.sentenceCountText)
                WorkbenchMetricCard(title: "Paragraphs", value: scene.paragraphCountText)
                WorkbenchMetricCard(title: "Characters", value: scene.characterCountText)
                WorkbenchMetricCard(title: "Encoding", value: scene.encodingText)
            }
...
private struct LibraryCorpusMetadataEditorSheetView: View {
    @Environment(\.dismiss) private var dismiss
    @Environment(\.wordZLanguageMode) private var languageMode
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            WorkbenchPaneCard(
                ...
                subtitle: t("这些字段会进入语料信息面板，也会为后续检索、筛选和导出打基础。", "These fields feed corpus info and prepare later filtering and exports.")
            ) {
                ...
                editorField(title: t("标签", "Tags"), text: $tagsText, prompt: t("多个标签用逗号分隔", "Separate multiple tags with commas"))
            }
